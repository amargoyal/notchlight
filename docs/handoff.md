...
asks for the raw macOS markers (`org.nspasteboard.ConcealedType`,
`TransientType`, `AutoGeneratedType`, and two password-manager types) so
those pastes are never read. Text over 20 KB is skipped; the history size is
a choice of 20/50/100; total bytes are capped at 2 MB and pins at 20; pins
survive the size cap and a plain clear but not "Clear everything". Copying an
item back is ignored by the poller. `~/.notchlight/clipboard.json` is written
0600. Preferences: `clipboardEnabled`, `clipboardHistorySize`,
`restClipboard`. `CompanionView` gains `'clipboard'`; IPC `clipboard:copy`,
`pin`, `remove`, `clear`, `pause`. The live face in `LiveCompanion.tsx` has
filter, pin, remove, pause and clear; the tab appears only while the history
is on. Gallery scenarios and `scripts/test-clipboard.mjs` cover the store and
