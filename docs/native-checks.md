...
| Apple Music | set Customize → Music → Player to Apple Music, play a track in Music | `spotify following Apple Music`, `spotify status … → ready`, `levels helper start` with `--bundle com.apple.Music` | artwork, title and bars follow Music; skips inside Music show at once |
...
| Bars in time | over AirPods, watch the bars against a track with a clear beat for a minute | `levels helper ready: {"ok":true,…"fps":60,"latencyMs":N,"offsetMs":0,"pipelineMs":P,"holdMs":H}` where `H` is `N` less `P` | bars land with the beat; if they are persistently early or late, set `levelsOffsetMs` in config.json (80 / -80), restart, and watch again |
