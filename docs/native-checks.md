...
| Bars in time | over AirPods, watch the bars against a track with a clear beat for a minute | `levels helper ready: {"ok":true,…"latencyMs":N,"offsetMs":0,"delayFrames":F}` | bars land with the beat; if they are persistently early or late, set `levelsOffsetMs` in config.json (80 / -80), restart, and watch again |
