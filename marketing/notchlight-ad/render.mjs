...
const failures=[];
async function page(){const p=await browser.newPage({viewport:{width:1080,height:1920},deviceScaleFactor:1});p.on('pageerror',e=>failures.push(e.message));await p.goto('http://127.0.0.1:4177/?render=1');await p.waitForFunction(()=>window.__ready&&window.__measurements);await p.evaluate(()=>Promise.all([...document.images].map(i=>i.decode().catch(()=>{}))));return p}
if(review){
