...
const dynamic=process.argv.includes('--dynamic'),short=dynamic||process.argv.includes('--short'),duration=short?15:36,prefix=dynamic?'dynamic-':short?'short-':'';
const review=process.argv.includes('--review'),fps=60,total=duration*fps;
await fs.mkdir(path.join(dir,'review'),{recursive:true});await fs.mkdir(path.join(dir,'exports'),{recursive:true});
const browser=await pw.chromium.launch({headless:true});
const failures=[];
async function page(){const p=await browser.newPage({viewport:{width:1080,height:1920},deviceScaleFactor:1});p.on('pageerror',e=>failures.push(e.message));await p.goto(`http://127.0.0.1:4177/${dynamic?'dynamic.html':short?'short.html':''}?render=1`);await p.waitForFunction(()=>window.__ready&&window.__measurements);for(let k=0;k<6;k++){const bad=await p.evaluate(()=>[...document.images].filter(i=>i.complete&&!i.naturalWidth).map(i=>{const u=i.src.split('?')[0];i.src=u+'?retry='+Math.random().toString(36).slice(2);return u}));if(k&&bad.length)console.log('retrying images:',bad.join(', '));await p.waitForFunction(()=>[...document.images].every(i=>i.complete),null,{timeout:20000});if(!bad.length)break;await new Promise(r=>setTimeout(r,300));}
...
if(review){
 const p=await page();const times=dynamic?[0,.8,2.8,3.6,3.75,3.95,4.23,4.25,4.45,4.7,4.9,5.1,7.85,10.35,13.3,14.98]:short?[0,.3,.8,1.8,2.8,3.6,4.55,5.9,6.6,7.2,7.85,8.4,9.3,10.35,11,11.8,12.6,13.3,14.98]:[0,1.8,3.5,5.5,7.7,9.7,12.5,13.8,15.95,18.8,20.5,21.75,22.7,25,26.9,28,30,33.5,35.9];const report=[];
 for(const t of times){await p.evaluate(t=>window.seek(t),t);await p.screenshot({path:path.join(dir,'review',`${prefix}frame-${t.toFixed(2)}.jpg`),type:'jpeg',quality:90});report.push(await p.evaluate(()=>window.__frame));}
 await fs.writeFile(path.join(dir,`review/${prefix}frames.json`),JSON.stringify({failures,measurements:await p.evaluate(()=>window.__measurements),frames:report},null,2));
 ...
 await Promise.all(Array.from({length:workers},async(_,i)=>{
  const p=await page(),out=path.join(dir,'exports',`${prefix}part-${i}.mp4`);
 ...
 const list=path.join(dir,`exports/${prefix}concat.txt`);await fs.writeFile(list,Array.from({length:4},(_,i)=>`file '${prefix}part-${i}.mp4'`).join('\n'));
 execFileSync(ff,['-hide_banner','-loglevel','error','-y','-f','concat','-safe','0','-i',list,'-c','copy','-movflags','+faststart',path.join(dir,`exports/Notchlight-${dynamic?'Dynamic-':''}Silent-${duration}s.mp4`)]);
 console.log(`Finished ${duration}-second 1080 × 1920 / 60 fps silent master.`);
}
await browser.close();
