...
SHORT=DYNAMIC or '--short' in sys.argv
track=json.loads((ROOT/'dynamic-track.json').read_text()) if DYNAMIC else None
DURATION=15 if SHORT else 36
...
if SHORT:
    # The dynamic cut restores a brief soundtrack pause and resume.
    if DYNAMIC:
        for name in ["pause","play"]:
            click(track[name],.1,.12);cue(track[name],name.capitalize())
    whoosh(.12,.44,.1);cue(.12,'Immediate notch unfold')
    click(3.05,.1,-.3);cue(3.05,'Music tab')
    for i,t in enumerate(track["scroll"] if DYNAMIC else [4.35,4.55,4.75]):
        add(t,tone(700+i*120,.09,45),.04,.3)
        click(t,.04,.3)
    cue(track['scroll'][0] if DYNAMIC else 4.35,'Volume: three ascending wheel ticks')
    whoosh(5.5,.26,.065,-1);cue(5.5,'Fold to resting notch')
    click(6.15,.1,.15);add(6.15,tone(230,.16,22),.08,.15);cue(6.15,'File lifted')
    whoosh(6.5,.5,.07);cue(6.5,'File carry')
    whoosh(7.05,.32,.075);cue(7.05,'Tray unfolds')
    add(7.72,tone(145,.28,21),.15,-.2);click(7.72,.08,-.2);cue(7.72,'File lands')
    click(8.85,.11,.1);whoosh(8.9,.3,.045);cue(8.85,'Clipboard tab')
    click(10.1,.085);add(10.13,tone(980,.22,23),.04);cue(10.1,'Copied: light pop')
    whoosh(11.55,.26,.065,-1);cue(11.55,'Everything folds away')
    whoosh(12.04,.45,.065)
    add(12.3,tone(220,.7,5),.055);add(12.34,tone(660,.7,5),.022)
    cue(12.3,'New logo resolves')
else:
...
output=ROOT/'exports'/f'Notchlight-{"Dynamic-" if DYNAMIC else ""}SFX-{DURATION}s.wav'
...
(ROOT/('sfx-cues-dynamic.json' if DYNAMIC else 'sfx-cues-15s.json' if SHORT else 'sfx-cues.json')).write_text(json.dumps(cues,indent=2)+'\n')
