...
const { chooseDisplayIds, geometryFor } = await import(pathToFileURL(file).href);
...
assert.deepEqual(geometryFor(display(), cfg(), probes([BUILT_IN])), { notchW: 185, notchH: 37 }, 'the cutout, raised to cover the menu bar it sits in');
assert.deepEqual(geometryFor(display(), cfg({ notchHeight: 'menu-bar' }), probes([BUILT_IN])), { notchW: 185, notchH: 37 }, 'the menu bar exactly');
assert.deepEqual(geometryFor(display(), cfg({ notchHeight: 'custom' }), probes([BUILT_IN])), { notchW: 185, notchH: 40 }, 'a figure of your own wins outright');
assert.deepEqual(geometryFor(display({ id: 3, workArea: { y: 30 } }), cfg(), probes([BUILT_IN])), { notchW: 200, notchH: 32 }, 'no cutout: the fallback width and the plain height');
assert.deepEqual(geometryFor(display({ id: 3, workArea: { y: 30 } }), cfg({ plainNotchHeight: 0 }), probes([BUILT_IN])), { notchW: 200, notchH: 30 }, 'a zero plain height falls back to the menu bar');

rmSync(out, { recursive: true, force: true });
console.log('Display checks passed: screen selection across laptop, docked, lid-shut and unplugged arrangements, and bar heights with and without a cutout.');
