...
export function useCompanion() {
  const available = !!window.claudeLight?.getCompanion;
  const [state, setState] = useState<CompanionSnapshot>({ preferences: { ...DEFAULT_COMPANION_PREFERENCES }, view: 'claude', files: [], music: { ...EMPTY_SPOTIFY }, notice: '' });
...
const views: CompanionView[] = ['claude','music','tray'];
const names = { claude: 'Claude', music: 'Music', tray: 'Tray' };
...
function LiveMusic({ live }: { live: LiveController }) {
  ...
  const command = (name: 'toggle' | 'next' | 'previous') => { void live.run(() => window.claudeLight.controlSpotify(name)); };
  const commitSeek = (position: number) => { setSeek(null); void live.run(() => window.claudeLight.controlSpotify('seek', position)); };
  if (music.status !== 'ready' || !track) {
    const open = music.status === 'not-running' || music.status === 'empty' && !music.busy;
    return <div className="mp-empty"><Icon name={music.status === 'error' || music.status === 'permission' ? 'warning' : 'music'} size={30}/><p>{music.busy ? 'Connecting to Spotify…' : music.status === 'disconnected' ? 'Your Spotify, closer.' : music.status === 'permission' ? 'Spotify needs your permission.' : open ? 'A little room for music.' : 'Spotify is taking a moment.'}</p><span>{music.message || 'Connect the Spotify desktop app to see and control what’s playing.'}</span><button className="mp-soft-button" disabled={music.busy} onClick={() => void live.run(() => open ? window.claudeLight.openSpotify() : window.claudeLight.connectSpotify())}>{open ? 'Open Spotify' : music.status === 'disconnected' ? 'Connect Spotify' : 'Reconnect Spotify'}</button></div>;
  }
  const position = seek ?? music.position;
  return <div className="mp-music"><div className="mp-track-row"><LiveArtwork live={live}/><div className="mp-track-meta"><div className="mp-source">Spotify <span>·</span> {music.playing ? 'Playing' : 'Paused'}</div><h2 title={track.title}>{track.title}</h2><p title={track.artist}>{track.artist}</p><div className="mp-transport"><button className="mp-icon-button" disabled={music.busy} aria-label="Previous track" onClick={() => command('previous')}><Icon name="back" size={22}/></button><button className="mp-play" disabled={music.busy} aria-label={music.playing ? 'Pause Spotify' : 'Play Spotify'} onClick={() => command('toggle')}><Icon name={music.playing ? 'pause' : 'play'} size={20}/></button><button className="mp-icon-button" disabled={music.busy} aria-label="Next track" onClick={() => command('next')}><Icon name="next" size={22}/></button></div></div></div><div className="mp-seek"><input aria-label="Track position" aria-valuetext={`${time(position)} of ${time(track.duration)}`} type="range" min="0" max={track.duration} value={position} disabled={music.busy || !track.duration} onChange={e => setSeek(Number(e.target.value))} onPointerUp={e => commitSeek(Number(e.currentTarget.value))} onPointerCancel={() => setSeek(null)} onKeyUp={e => { if (['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','Home','End','PageUp','PageDown'].includes(e.key)) commitSeek(Number(e.currentTarget.value)); }}/><div><span>{time(position)}</span><span>−{time(Math.max(0,track.duration-position))}</span></div></div></div>;
}
function FileButton({ file, live, selected, onSelect }: { file: ShelfFile; live: LiveController; selected: boolean; onSelect: () => void }) {
  return <div className={`mp-shelf-item ${selected ? 'is-selected' : ''}`}><button className="mp-file-button" aria-label={`Select ${file.name}${file.unavailable ? ', unavailable' : ''}`} aria-pressed={selected} draggable={!file.unavailable} onDragStart={e => { e.preventDefault(); window.claudeLight.startFileDrag(file.id); }} onClick={onSelect} onDoubleClick={() => void live.run(() => window.claudeLight.revealFile(file.id))}><FileThumb file={file} small={live.state.preferences.thumbnails === 'small'} sample={false}/><span title={file.name}>{file.name}</span><small>{file.unavailable ? 'Unavailable' : file.size}</small></button><button className="mp-remove" aria-label={`Remove ${file.name} from Tray`} onClick={() => void live.run(() => window.claudeLight.removeFile(file.id))}><Icon name="close" size={12}/></button></div>;
}
function LiveTray({ live, dragging }: { live: LiveController; dragging: boolean }) {
  ...
  return <div className={`mp-tray ${dragging ? 'is-drop-target' : ''}`}><div className="mp-tray-heading"><span>{dragging ? 'Drop it here. Pick it up later.' : 'A place between places.'}</span><button className="mp-text-button" onClick={() => void live.run(() => window.claudeLight.pickFiles())}>Add files…</button></div>{!live.state.files.length ? <div className="mp-empty mp-empty-tray"><Icon name="tray" size={32}/><p>Keep it here for a moment.</p><span>Drop files from Finder. They stay in their original location.</span><button className="mp-soft-button" onClick={() => void live.run(() => window.claudeLight.pickFiles())}>Choose files…</button></div> : <div className={`mp-file-grid ${live.state.preferences.thumbnails}`} aria-label="Files in Tray">{live.state.files.map(file => <FileButton key={file.id} file={file} live={live} selected={file.id === selected} onSelect={() => setSelected(file.id)}/>)}</div>}<div className="mp-tray-footer"><span>{file?.name || 'Drag out to another app. Originals stay put.'}</span><button className="mp-text-button" disabled={!file || file.unavailable} onClick={() => file && void live.run(() => window.claudeLight.saveFileCopy(file.id))}>Save copy… <Icon name="arrow" size={13}/></button></div></div>;
}
