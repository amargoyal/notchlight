...
export function useCompanion() {
  ...
  const [geometry, setGeometry] = useState<{ notchW: number; notchH: number; fullscreen: boolean } | null>(null);
  useEffect(() => {
  ...
  const { fullscreen = false, ...measurements } = geometry ?? {};
  return { available, state, snapshot: geometry ? { ...snapshot, ...measurements } : snapshot, fullscreen, ready, error, run };
}
...
function LiveMusic({ live }: { live: LiveController }) {
  ...
  if (music.status !== 'ready' || !track) {
    ...
    const name = live.state.preferences.musicPlayer === 'auto' && music.status === 'disconnected' ? 'music' : PLAYER_NAMES[music.player];
    return <div className="mp-empty"><Icon name={music.status === 'error' || music.status === 'permission' ? 'warning' : 'music'} size={30}/><p>{music.busy ? `Connecting to ${name}…` : music.status === 'disconnected' ? `Your ${name}, closer.` : music.status === 'permission' ? `${name} needs your permission.` : open ? 'A little room for music.' : `${name} is taking a moment.`}</p><span>{music.message || `Connect ${live.state.preferences.musicPlayer === 'auto' ? 'Spotify or Apple Music' : name} to see and control what’s playing.`}</span><button className="mp-soft-button" disabled={music.busy} onClick={() => void live.run(() => open ? window.notchlight.openSpotify() : window.notchlight.connectSpotify())}>{open ? `Open ${PLAYER_NAMES[music.player]}` : music.status === 'disconnected' ? 'Connect' : 'Reconnect'}</button></div>;
  }
  const playing = expected ?? music.playing;
  const smart = live.state.smartShuffle;
  // The pick is only the current track's; a stale answer never dresses the next one.
...
export function CompanionSurface({ live, open, hovering, keyboard = false, onBox, onCustomize }: { live: LiveController; open: boolean; hovering: boolean; keyboard?: boolean; onBox?: (r: {x:number;y:number;w:number;h:number}) => void; onCustomize: () => void }) {
  ...
  // Before the tab row, which reads them: `tabs` is a const holding JSX, so it
  // runs the moment it is assigned, and anything it touches has to exist by then.
  const { calendar } = live.state;
  ...
  const peekResting = hud || power || hideMedia || !peek ? undefined
    : <RestingWings notchW={snapshot.notchW} height={snapshot.notchH} parts={[{
  ...
  const resting = hideAll ? <div style={{ width: snapshot.notchW, height: snapshot.notchH }}/>
    : hudResting ?? powerResting ?? peekResting ?? (parts.length ? <RestingWings notchW={snapshot.notchW} height={snapshot.notchH} parts={parts}/> : snapshot.sessions.length ? (hovering ? <Stubs snap={snapshot}/> : <div style={{width:snapshot.notchW,height:snapshot.notchH}}/>) : undefined);
  ...
  const active = !hideAll && (!snapshot.dormant || parts.length > 0 || keyboard || hud !== null || power !== null || peek !== null);
