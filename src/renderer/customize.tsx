    ...
    <main className="settings-main"><header className="settings-header"><div><span>Customize</span><h1>{current.label}</h1><p>{current.description}</p></div><button className="mp-text-button settings-reset" disabled={live.available && !live.ready} onClick={() => { if (live.available) void live.run(() => window.claudeLight.updatePreferences(DEFAULT_PREFERENCES)); else dispatch({ type: 'reset' }); setSection('appearance'); }}><Icon name="reset" size={15}/>{live.available ? 'Reset preferences' : 'Reset Preview'}</button></header>
      <div className="settings-workspace"><fieldset className="settings-controls" disabled={live.available && !live.ready}>
        {section === 'appearance' && <>
          ...
          <Group title="While resting"><Toggle title="Claude" description="Status light and buddy while a session runs." value={prefs.restClaude} onChange={v => pref('restClaude', v)}/><Toggle title="Music" description="Album art and playback while a track is ready." value={prefs.restMusic} onChange={v => pref('restMusic', v)}/><Toggle title="Tray" description="A count of what you have set aside." value={prefs.restTray} onChange={v => pref('restTray', v)}/><p className="settings-hint">Faces you switch on share the bar when they have something to show. Claude still surfaces a dot when it needs you.</p></Group>
