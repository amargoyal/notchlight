...
const sections: { id: Section; label: string; description: string }[] = [
  ...
  { id: 'clipboard', label: 'Clipboard', description: 'What you copied, still within reach.' }
];
...
function App() {
  ...
  const navigate = (id: Section) => { setSection(id); if (id !== 'appearance' && (id !== 'clipboard' || prefs.clipboardEnabled)) { if (live.available) void live.run(() => window.notchlight.setView(id)); else dispatch({ type: 'view', view: id }); } };
  return <div className={`customize-app theme-${prefs.theme}`}>
    ...
    <main className="settings-main"><header className="settings-header"><div><span>Customize</span><h1>{current.label}</h1><p>{current.description}</p></div><button className="mp-text-button settings-reset" disabled={live.available && !live.ready} onClick={() => { if (live.available) void live.run(() => window.notchlight.updatePreferences(DEFAULT_PREFERENCES)); else dispatch({ type: 'reset' }); setSection('appearance'); }}><Icon name="reset" size={15}/>{live.available ? 'Reset preferences' : 'Reset Preview'}</button></header>
      <div className="settings-workspace"><fieldset className="settings-controls" disabled={live.available && !live.ready}>
        ...
        {section === 'music' && <><Group title="Now playing"><Toggle title="Show album artwork" description="Give each track a familiar face." value={prefs.artwork} onChange={v => pref('artwork', v)}/><Toggle title="Move with the music" description={live.available ? 'Bars in the collapsed wing follow what Spotify is actually playing.' : 'A quiet rhythm in the collapsed wing.'} value={prefs.visualizer} onChange={v => pref('visualizer', v)}/><Choice label="Bars" value={prefs.equalizerLayout} options={[{ value: 'rising', label: 'Rising' }, { value: 'mirrored', label: 'Mirrored' }]} onChange={v => pref('equalizerLayout', v)}/><Toggle title="Glow in the artwork’s colour" description="A soft light behind the artwork and the bars, taken from the record sleeve." value={prefs.artworkGlow} onChange={v => pref('artworkGlow', v)}/><Toggle title="Breathe with the bass" description="The small artwork moves with the low end while the bars are live. Still under Reduce motion." value={prefs.artworkPulse} onChange={v => pref('artworkPulse', v)}/><Toggle title="Smart Shuffle picks" description="Mark a track Spotify slipped into the playlist, with + to keep it and × to move on. Needs the account below." value={prefs.smartShuffle} onChange={v => pref('smartShuffle', v)}/></Group>{!live.available && <Group title="Try a moment"><label className="settings-select">Player state<select value={state.music.missingArtwork ? 'missing' : state.music.source} onChange={e => dispatch({ type: 'music-state', source: e.target.value === 'missing' ? 'ready' : e.target.value as PreviewState['music']['source'], missingArtwork: e.target.value === 'missing' })}><option value="ready">Ready to play</option><option value="empty">Nothing playing</option><option value="missing">Missing artwork</option><option value="unavailable">Player unavailable</option></select></label><p className="settings-hint">Play, pause, skip, and seek through an original sample playlist. No audio will play.</p></Group>}{live.available && <Group title="Player"><Choice label="Follow" value={live.state.preferences.musicPlayer} options={[{ value: 'spotify', label: 'Spotify' }, { value: 'apple', label: 'Apple Music' }, { value: 'auto', label: 'Whichever is open' }]} onChange={v => void live.run(() => window.notchlight.updatePreferences({musicPlayer:v}))}/><p className="settings-hint">{live.state.music.message || (live.state.music.status === 'ready' ? `Connected to ${live.state.music.player === 'apple' ? 'Apple Music' : 'Spotify'} on this Mac.` : 'Connect a player to show real music in the notch.')}</p><button className="mp-soft-button" disabled={live.state.music.busy} onClick={() => void live.run(() => window.notchlight.connectSpotify())}>{live.state.preferences.spotifyEnabled ? 'Reconnect' : 'Connect'}</button>{live.state.preferences.spotifyEnabled && <button className="mp-text-button" style={{display:'block',marginTop:12}} onClick={() => void live.run(() => window.notchlight.updatePreferences({spotifyEnabled:false}))}>Disconnect</button>}<p className="settings-hint">macOS may ask for Automation access to the player. No account sign-in is needed here. Whichever is open starts with Spotify and moves to Apple Music while Spotify is not running.</p></Group>}{live.available && <Group title="Spotify account"><p className="settings-hint">Smart Shuffle picks come from Spotify’s Web API, which needs an app of your own: create one at developer.spotify.com/dashboard, add the address below as a Redirect URI, and paste its Client ID here. Playback needs none of this.</p><div className="settings-code"><code>{SPOTIFY_REDIRECT_URI}</code><button className="mp-text-button" onClick={() => void navigator.clipboard.writeText(SPOTIFY_REDIRECT_URI)}>Copy</button></div><ClientIdField value={live.state.preferences.spotifyClientId} onSave={id => void live.run(() => window.notchlight.updatePreferences({ spotifyClientId: id }))}/><p className="settings-hint" role="status">{describeAccount(live.state.smartShuffle.account)}</p>{live.state.smartShuffle.account.status !== 'off' && <button className="mp-soft-button" disabled={live.state.smartShuffle.account.status === 'signing-in'} onClick={() => void live.run(() => live.state.smartShuffle.account.status === 'ready' ? window.notchlight.signOutSpotify() : window.notchlight.signInSpotify())}>{live.state.smartShuffle.account.status === 'ready' ? 'Sign out' : live.state.smartShuffle.account.status === 'signing-in' ? 'Waiting for your browser…' : 'Sign in to Spotify'}</button>}</Group>}{live.available && live.state.preferences.spotifyEnabled && <Group title="Audio capture"><p className="settings-hint" role="status">{describeCapture(live.state.capture, live.state.music, live.state.preferences)}</p><p className="settings-hint">Capture is separate from the Spotify connection: track details and playback controls keep working when the bars cannot. Nothing is recorded; Spotify’s output is reduced to five numbers and dropped.</p></Group>}</>}
        ...
        {section === 'clipboard' && <>
          <Group title="History">
            <Toggle title="Keep a clipboard history" description="Off until you say so. Text you copy is kept on this Mac, in a file only you can read. Nothing is sent anywhere." value={prefs.clipboardEnabled} onChange={v => { pref('clipboardEnabled', v); if (v) { if (live.available) void live.run(() => window.notchlight.setView('clipboard')); else dispatch({ type: 'view', view: 'clipboard' }); } }}/>
            <Choice label="Remember up to" value={prefs.clipboardHistorySize} options={[{ value: '20', label: '20 items' }, { value: '50', label: '50 items' }, { value: '100', label: '100 items' }]} onChange={v => pref('clipboardHistorySize', v)}/>
            <Toggle title="Rest on the bar" description="A count and the kind of the latest item on the collapsed notch." value={prefs.restClipboard} onChange={v => pref('restClipboard', v)}/>
            <p className="settings-hint">Passwords and one-time codes marked concealed or transient by their apps are never read. Items over 20 KB are skipped; the whole history stays under 2 MB and up to 20 items can be pinned. Images are not kept yet.</p>
          </Group>
          {live.available && prefs.clipboardEnabled && <Group title="Right now">
            <p className="settings-hint" role="status">{live.state.clipboard.paused ? 'Capture is paused.' : `Watching the clipboard. ${live.state.clipboard.items.length} ${live.state.clipboard.items.length === 1 ? 'item' : 'items'} remembered, ${live.state.clipboard.items.filter(i => i.pinned).length} pinned.`}</p>
            <button className="mp-soft-button" onClick={() => void live.run(() => window.notchlight.pauseClipboard(!live.state.clipboard.paused))}>{live.state.clipboard.paused ? 'Resume capture' : 'Pause capture'}</button>
            <button className="mp-text-button" style={{marginLeft:12}} disabled={!live.state.clipboard.items.length} onClick={() => void live.run(() => window.notchlight.clearClipboard(false))}>Clear history, keep pins</button>
            <button className="mp-text-button" style={{marginLeft:12}} disabled={!live.state.clipboard.items.length} onClick={() => void live.run(() => window.notchlight.clearClipboard(true))}>Clear everything</button>
          </Group>}
          {!live.available && <Group title="Try it out"><p className="settings-hint">Switch the history on to see the Clipboard face in the preview. Click a sample item to copy it again; pin what you want to keep.</p></Group>}
        </>}
